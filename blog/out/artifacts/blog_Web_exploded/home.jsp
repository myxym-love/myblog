...
<section class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-4 py-12">
    <div class="w-full text-center pb-8">
        ...
        <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-6 flex flex-wrap">
            <c:forEach items="${pb.list}" var="article" varStatus="s">
            <div class="bg-white rounded-lg p-6">
                <div class="flex items-center space-x-6 mb-4">
                    <img class="h-28 w-28 object-cover object-center rounded-full"
                         src="./img/head_img.png" alt="photo">
                    <div>
                        <a class="title" href="${pageContext.request.contextPath}/ArticleServlet?article_id=${article.article_id}"><p class="text-xl text-gray-700 font-normal mb-1">标题:${article.article_title}</p></a>
                        <p class="text-base text-blue-600 font-normal">简介:${article.article_ind}</p>
                    </div>
                </div>
                <div class="overflow-clip overflow-hidden ">
                    <p class="text-gray-400 leading-loose font-normal text-base h-40">${article.article_context}</p>
                </div>
            </div>
            </c:forEach>
        </div>
