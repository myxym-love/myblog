...
public class ArticleDaoImpl implements ArticleDao {
    ...
    public List<ArticleByName> findAll() {
        //使用JDBC操作数据库
        String sql = "SELECT article_id,user_id,article_title,article_context,article_date,article_ind,u.user_realname FROM article as a,user as u where a.user_id = u.id";
        List<ArticleByName> articles = template.query(sql, new BeanPropertyRowMapper<>(ArticleByName.class));
        return articles;
    }
    ...
    @Override
    public List<ArticleByName> findByPage(int start, int rows, String words) {
        String sql = "SELECT article_id,user_id,article_title,article_context,article_date,article_ind,u.user_realname FROM article as a,user as u where 1=1 and a.user_id = u.id";
        StringBuilder sb = new StringBuilder(sql);
        if(words!=null&&!"".equals(words)){
            sb.append(" and article_title like ? ");
        }
        sb.append(" LIMIT ? , ? ");
        List<ArticleByName> list;
        if(words!=null&&!"".equals(words)){
            words="%"+words+"%";
            list = template.query(sb.toString(), new BeanPropertyRowMapper<>(ArticleByName.class),words,start,rows);

        }else {
            list = template.query(sb.toString(), new BeanPropertyRowMapper<>(ArticleByName.class),start,rows);
        }
        return list;
    }
/*
